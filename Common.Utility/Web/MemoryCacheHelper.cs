...
namespace Common.Utility.Web
{
    public class MemoryCacheHelper
    {
        ...
        /// <param name="expireAbsolute">绝对过期时长</param>
        /// <returns></returns>
        public bool Set(string key, object value, TimeSpan expiresSliding, TimeSpan expireAbsolute)
        {
            if (key == null)
                throw new ArgumentNullException(nameof(key));
            if (value == null)
            ...
            Cache.Set(key, value,
                new MemoryCacheEntryOptions().SetSlidingExpiration(expiresSliding)
                    .SetAbsoluteExpiration(expireAbsolute));
            return Exists(key);
        }
