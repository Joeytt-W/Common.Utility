...
namespace Commom.Utility
{
    ...
    public class LogChip : IDisposable
    {
        //日志对象的缓存队列
        ...
        /// <param name="t">日志文件创建方式的枚举</param>
        public LogChip(string p, LogType t)
        {
            if (msgs == null)
            {
                state = true;
                ...
                thread.IsBackground = true;//确保主线程退出后该线程被关闭
                thread.Start();
            }
        }
    ...
    public class Msg
    {
        ...
        {
            return Type.ToString() + "\t" + datetime.ToString() + "\t" + text + "\n";
        }
    }
